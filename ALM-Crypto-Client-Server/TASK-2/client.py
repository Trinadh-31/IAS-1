...
# Same educational S-DES byte implementation used in TASK-1.
...
with socket.socket(socket.AF_INET,socket.SOCK_STREAM) as client:
    ...
    cipher = bytes(enc(b) for b in data)
    print('CLIENT -> SERVER')
    print('Plaintext file size:', len(data), 'bytes')
    print('Ciphertext preview:', cipher[:64].hex())
    client.sendall(b'1MB' + len(cipher).to_bytes(4,'big') + hashlib.sha256(data).digest() + cipher)
    response=client.recv(1024).decode()
    print(response)

    marker=client.recv(4)
    size=int.from_bytes(client.recv(4),'big')
    cipher10=recv_all(client,size)
    plain10=bytes(dec(b) for b in cipher10)
    os.makedirs('files',exist_ok=True)
    with open('files/received_10KB.txt','wb') as f: f.write(plain10)
    print('\nSERVER -> CLIENT')
    print('Ciphertext file size:', len(cipher10), 'bytes')
    print('Plaintext file size:', len(plain10), 'bytes')
    print('Ciphertext preview:', cipher10[:64].hex())
    print('Plaintext preview:', plain10[:64])
    print('SHA-256:', hashlib.sha256(plain10).hexdigest())
